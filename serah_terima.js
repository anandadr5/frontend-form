 // Variabel untuk validasi ukuran file
      ...
      const SCRIPT_URL =
        "https://pengawasan-tambahspk.onrender.com/api/form?form=serah_terima";
      const GET_URL = "https://pengawasan-tambahspk.onrender.com/api/form";
      ...
      const closeErrorPopupButton =
        document.getElementById("close-error-popup");

      async function loadKodeUlok() {
        let email = getParameterByName("assigned_email"); 
        if (!email) {
          email = getParameterByName("email");
        }
        
        if (!email) {
          showPopup("Email tidak ditemukan di URL");
          return;
        }
        ...
        try {
          ...
          const select = document.getElementById("kode_ulok");
          select.innerHTML = '<option value="">-- Pilih Kode Ulok --</option>';

          if (result.data && Array.isArray(result.data)) {
            ulokDataStore = result.data;
            ...
            console.error("Data kode ulok tidak valid:", result);
            showPopup("Gagal memuat data kode ulok.");
          }
        } catch (err) {
          console.error("Gagal memuat mode ulok:", err);
          showPopup("Terjadi kesalahan: " + err.message);
        }
      }

      function handleUlokChange() {
        const selectedKode = document.getElementById("kode_ulok").value;
        ...
        document.querySelectorAll(".kelompok").forEach(section => {
            section.querySelectorAll(".pekerjaan-item").forEach(updateItemState);
            updateSectionColor(section);
        });
        checkFinalStatus();
        ...
        const selectedData = ulokDataStore.find(
          (item) => item.kode === selectedKode
        );

        if (selectedData) {
          lokasiInput.value = selectedData.alamat || "";
          cabangInput.value = selectedData.cabang || "";
        }

        loadFormFromLocalStorage(selectedKode);
      }

      // TAMBAHKAN FUNGSI INI (setelah pekerjaanData dan sebelum form.addEventListener)
function getSelectedItems() {
  ...
  itemDivs.forEach(itemDiv => {
    ...
    const selectAda = itemDiv.querySelector(`select[name="${kode}_ada"]`);
    
    if (! selectAda) return;
    
    const adaTidak = selectAda.value;
    
    // Hanya simpan jika user memilih sesuatu (tidak kosong)
    if (adaTidak && adaTidak !== "") {
      const itemData = {
        kode:  kode,
        area: area,
        ada_tidak: selectAda.options[selectAda.selectedIndex]?.textContent || "-",
        sesuai_design_standart: "-",
        kualitas_kerapihan: "-",
        spesifikasi_material: "-",
        diterima_tidak: "-"
      };

      // Ambil nilai field lainnya hanya jika ada/tidak = "ada"
      if (adaTidak. toLowerCase() === "ada") {
        const selectDesain = itemDiv.querySelector(`select[name="${kode}_desain"]`);
        const selectKualitas = itemDiv.querySelector(`select[name="${kode}_kualitas"]`);
        const selectMaterial = itemDiv.querySelector(`select[name="${kode}_material"]`);
        const selectDiterima = itemDiv.querySelector(`select[name="${kode}_diterima"]`);

        if (selectDesain && selectDesain.value) {
          itemData.sesuai_design_standart = selectDesain.options[selectDesain.selectedIndex]?.textContent || "-";
        }
        if (selectKualitas && selectKualitas.value) {
          itemData.kualitas_kerapihan = selectKualitas. options[selectKualitas.selectedIndex]?.textContent || "-";
        }
        if (selectMaterial && selectMaterial.value) {
          itemData.spesifikasi_material = selectMaterial.options[selectMaterial. selectedIndex]?.textContent || "-";
        }
        if (selectDiterima && selectDiterima.value) {
          itemData.diterima_tidak = selectDiterima.options[selectDiterima.selectedIndex]?. textContent || "-";
        }
      }
  ...
  const statusSerah = document.getElementById("status_serah").value;
  if (statusSerah === "") {
    ...
    for (const section of allSections) {
      ...
      for (const item of items) {
        ...
        if (firstInvalidElement) {
          document.querySelectorAll(".kelompok").forEach(s => s.classList.add("collapsed"));
          section.classList.remove("collapsed");

          item.scrollIntoView({ behavior: "smooth", block: "center" });
          
          showErrorPopup(
            "Masih ada item yang belum diisi.  Harap periksa kembali.",
            null
          );
  ...
  const payload = {
    ...
    pekerjaan: "", // Jika ada field pekerjaan, isi sesuai kebutuhan
    kode_ulok: document.getElementById("kode_ulok").value,
    kategori_lokasi: "", // Tambahkan jika ada
    pic_nama: getParameterByName("assigned_nama") || getParameterByName("nama"),
  ...
  // Validasi minimal ada 1 item dipilih
  if (payload. items_dipilih.length === 0) {
    showErrorPopup("Harap pilih minimal 1 item pekerjaan!", null);
    ...
    if (fileInput && fileInput.files.length > 0 && ! fileInput.disabled) {
      ...
      try {
        ...
        fotoUpload.push({
          item: kode,
          area: area,
          base64: base64, // Kirim full base64 (dengan header)
        });
      } catch (error) {
  ...
  try {
    ...
    if (response.ok && result.status === "success") {
      ...
      const submittedKodeUlok = payload.kode_ulok; 
      if (submittedKodeUlok) {
        const storageKey = `autosave_serahTerima_${submittedKodeUlok}`;
        localStorage.removeItem(storageKey); 
      }

      form.reset();
      document.querySelectorAll(".kelompok").forEach(updateSectionColor);
      checkFinalStatus();
    } else {
    ...
    submitButton. disabled = false;
      ...
      function showErrorPopup(message, url) {
      ...
      function setupFileSizeValidation() {
        ...
        fileInputs.forEach((input) => {
          input.addEventListener("change", function () {
            ...
            const itemDiv = this.closest(".pekerjaan-item");
            const section = this.closest(".kelompok");
            if (itemDiv && section) {
              updateSectionColor(section);
              checkFinalStatus();
            }
      ...
      function saveFormToLocalStorage() {
        const form = document.getElementById("laporan-form");
        const kodeUlok = document.getElementById("kode_ulok").value;
        ...
        const data = {};
        form.querySelectorAll("select, input[type='text'], input[type='date']").forEach(el => {
          if (el.name) {
            data[el.name] = el.value;
          }
        });
        
        const storageKey = `autosave_serahTerima_${kodeUlok}`;
        localStorage.setItem(storageKey, JSON.stringify(data));
      }

      function loadFormFromLocalStorage(kodeUlok) {
        if (!kodeUlok) {
          return; 
        }
        ...
        if (savedData) {
          ...
          if (hasRestoredData) {
            ...
            showErrorPopup(
              "Data Anda sebelumnya untuk kode ulok ini berhasil dipulihkan. PERHATIAN: FOTO HARUS DI-UPLOAD ULANG.",
              null
            );
          }
        }
